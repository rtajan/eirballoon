...
::step(const std::complex<R> *X_N1, R* MU, std::complex<R>* Y_N1, B* B_N1)
{
	*MU = this->mu;

	const R* R_X_N1 = reinterpret_cast<const R*>(X_N1);
	R* R_Y_N1 = reinterpret_cast<R*>(Y_N1);

	farrow_flt.step( R_X_N1, R_Y_N1);
...
::interpolation_control()
{
	...
	if (this->is_strobe == 1) // Update mu if a strobe
	{
		...
		this->farrow_flt.set_mu(1-this->mu);
