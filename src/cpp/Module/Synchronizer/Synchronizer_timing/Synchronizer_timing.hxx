...
// {
// 	// call module set_n_frame
// 	aff3ct::module::Module::set_n_frames(n_frames);
// 	this->underflow_cnt.resize(this->n_frames, 0.0f);
// 	this->output_buffer.resize(output_buffer((N/osf*3)*this->n_frames, 0.0f);
// }
...
::synchronize(const std::vector<R,AR>& X_N1, std::vector<R,AR>& MU, std::vector<R,AR>& Y_N1, std::vector<B,AB>& B_N1, const int frame_id)
{
	...
	if (this->N_in/2*this->n_frames != (int)MU.size())
	{
		...
		message << "'MU.size()' has to be equal to  'N/2' * 'n_frames' ('MU.size()' = " << MU.size()
		        << ", 'N/2' = " << this->N_in/2 << ", 'n_frames' = " << this->n_frames << ").";
		throw tools::length_error(__FILE__, __LINE__, __func__, message.str());
	}
